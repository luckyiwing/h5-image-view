...
class ImageCache {
    ...
    async ensureCacheSpace(requiredSize) {
        ...
        // 检查内存限制
        while (this.currentMemoryUsage + requiredSize > this.maxMemorySize && this.cache.size > 0) {
            this.removeLeastRecentlyUsed();
        }
    }
...
class PerformanceMonitor {
    ...
    getPerformanceReport() {
        ...
        // 计算平均API调用时间
        const avgApiCallTime = this.metrics.apiCallTimes.length > 0
            ? this.metrics.apiCallTimes.reduce((sum, item) => sum + item.callTime, 0) / this.metrics.apiCallTimes.length
            : 0;

        // 计算缓存命中率
        const totalCacheRequests = this.metrics.cacheHitRate.hits + this.metrics.cacheHitRate.misses;
        const cacheHitRate = totalCacheRequests > 0
            ? (this.metrics.cacheHitRate.hits / totalCacheRequests * 100).toFixed(2)
            : 0;
...
class EventOptimizer {
    ...
    addEventListener(target, type, listener, options = {}) {
        ...
        console.log('添加优化事件监听器:', { type, passive: optimizedOptions.passive });
    }
    ...
    removeEventListener(target, type, listener) {
        ...
        if (listeners) {
            const index = listeners.findIndex(item => item.originalListener === listener);
            if (index !== -1) {
                const listenerInfo = listeners[index];
                target.removeEventListener(type, listenerInfo.listener, listenerInfo.options);
    ...
    wrapListener(listener, eventType) {
        return (event) => {
            ...
            try {
                ...
                // 如果执行时间过长，发出警告
                if (executionTime > 16) { // 16ms 约等于 60fps
                    console.warn(`事件监听器执行时间过长 (${eventType}): ${executionTime.toFixed(2)}ms`);
                }
            }
        };
    }
...
class EnhancedPreloadStrategy extends PreloadStrategy {
    ...
    async adaptivePreload() {
        try {
            ...
            // 根据命中率调整预加载数量
            if (hitRate < this.adaptiveThreshold && this.preloadCount < this.maxPreloadCount) {
                // 命中率低，增加预加载数量
                this.preloadCount = Math.min(this.preloadCount + 1, this.maxPreloadCount);
                ...
                this.preloadCount = Math.max(this.preloadCount - 1, this.minPreloadCount);
                this.stats.adaptiveAdjustments++;
    ...
    getPreloadStats() {
        const cacheStats = this.imageCache.getStats();
        const totalRequests = this.stats.cacheHits + this.stats.cacheMisses;
        const hitRate = totalRequests > 0 ? (this.stats.cacheHits / totalRequests * 100).toFixed(2) : '0.00';
    ...
    optimizePreloadCount() {
        try {
            ...
            // 基于命中率和内存使用情况优化
            if (hitRate < 50 && memoryUsage < 70 && this.preloadCount < this.maxPreloadCount) {
                // 命中率低且内存充足，增加预加载
                this.preloadCount = Math.min(this.preloadCount + 2, this.maxPreloadCount);
